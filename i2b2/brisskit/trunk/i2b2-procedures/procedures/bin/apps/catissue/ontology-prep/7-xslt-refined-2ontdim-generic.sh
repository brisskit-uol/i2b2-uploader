...
#
# USAGE: 7-xslt-refined-2ontdim job-name 
# Where: 
...
print_xslt_refined_2ontdim_usage() {
   echo " USAGE: 7-xslt-refined-2ontdim.sh job-name"
   echo " Where:"
...
#MAIN_REFINED_METADATA_CATISSUE_FILE_NAME

P_REFINED_METADATA_DIRECTORY=c-refined-metadata
P_METADATA_SQL_DIRECTORY=e-ontology-sql
P_MAIN_REFINED_METADATA_FILE_NAME=civi-metadata.xml
...
INPUT_DIR=$I2B2_PROCEDURES_WORKSPACE/$JOB_NAME/$P_REFINED_METADATA_DIRECTORY
#
# And it must exist!
if [ ! -d $INPUT_DIR ]
then
...
#===========================================================================
print_banner "xslt-refined-2ontdim" $JOB_NAME $LOG_FILE 

#
# And the output directory is...
OUTPUT_DIR=$I2B2_PROCEDURES_WORKSPACE/$JOB_NAME/$P_METADATA_SQL_DIRECTORY/$DB_TYPE/ontdim
#
...
STYLESHEET=$I2B2_PROCEDURES_HOME/xslt/$DB_TYPE/ont_dim_${DB_TYPE}.xsl
...
$JAVA_HOME/bin/java \
        -cp $(for i in $I2B2_PROCEDURES_HOME/lib/*.jar ; do echo -n $i: ; done). \
         ...
         -xsl:$STYLESHEET \
         -s:$INPUT_DIR/$P_MAIN_REFINED_METADATA_FILE_NAME \
...
print_footer "xslt-refined-2ontdim" $JOB_NAME $LOG_FILE
