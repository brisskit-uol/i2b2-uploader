/*========================================
  Postgres SQL for project work tables 
  
  Substitutions required for:
  	<WORK_SCHEMA_NAME>
	<WORK_USER_NAME>
	<WORK_PASSWORD>
  	<PROJECT_ID>
  
  ========================================*/

create schema <WORK_SCHEMA_NAME>;

create user <WORK_USER_NAME> password '<WORK_PASSWORD>';

 
set schema '<WORK_SCHEMA_NAME>';

CREATE TABLE WORKPLACE ( 
	C_NAME     	VARCHAR(255) NOT NULL,
	C_USER_ID	VARCHAR(255) NOT NULL,
	C_GROUP_ID	VARCHAR(255) NOT NULL,
	C_SHARE_ID	VARCHAR(255) NULL,
	C_INDEX  	VARCHAR(255) NOT NULL,
	C_PARENT_INDEX  	VARCHAR(255) NULL,
	C_VISUALATTRIBUTES   	CHAR(3) NOT NULL,
	C_PROTECTED_ACCESS    	CHAR(1) NULL,
	C_TOOLTIP      	VARCHAR(255) NULL,
	C_WORK_XML      	text NULL,
	C_WORK_XML_SCHEMA      	text NULL,
	C_WORK_XML_I2B2_TYPE      	VARCHAR(255) NULL,
	C_ENTRY_DATE   	timestamp NULL,
	C_CHANGE_DATE  	timestamp NULL,
	C_STATUS_CD    	CHAR(1) NULL,
 CONSTRAINT WORKPLACE_PK PRIMARY KEY(C_INDEX)  
	);


CREATE TABLE WORKPLACE_ACCESS ( 
	C_TABLE_CD   	VARCHAR(255) NOT NULL,
	C_TABLE_NAME      VARCHAR(255) NOT NULL,
	C_PROTECTED_ACCESS    	CHAR(1) NULL,
	C_HLEVEL	INT NOT NULL,
	C_NAME     	VARCHAR(255) NOT NULL,
	C_USER_ID	VARCHAR(255) NOT NULL,
	C_GROUP_ID	VARCHAR(255) NOT NULL,
	C_SHARE_ID	VARCHAR(255) NULL,
	C_INDEX  	VARCHAR(255) NOT NULL,
	C_PARENT_INDEX  	VARCHAR(255) NULL,
	C_VISUALATTRIBUTES   	CHAR(3) NOT NULL,
	C_TOOLTIP      	VARCHAR(255) NULL,
	C_ENTRY_DATE   	timestamp NULL,
	C_CHANGE_DATE  	timestamp NULL,
	C_STATUS_CD    	CHAR(1) NULL,
 CONSTRAINT WORKPLACE_ACCESS_PK PRIMARY KEY(C_INDEX) 
	);
	
INSERT INTO WORKPLACE_ACCESS(C_TABLE_CD, C_TABLE_NAME, C_PROTECTED_ACCESS, C_HLEVEL, C_NAME, C_USER_ID, C_GROUP_ID, C_SHARE_ID, C_INDEX, C_PARENT_INDEX, C_VISUALATTRIBUTES, C_TOOLTIP)
  VALUES('<PROJECT_ID>', 'WORKPLACE','N', 0, 'SHARED', 'shared', 'demo', 'Y', 100, NULL, 'CA', 'SHARED');

INSERT INTO WORKPLACE_ACCESS(C_TABLE_CD, C_TABLE_NAME, C_PROTECTED_ACCESS, C_HLEVEL, C_NAME, C_USER_ID, C_GROUP_ID, C_SHARE_ID, C_INDEX, C_PARENT_INDEX, C_VISUALATTRIBUTES, C_TOOLTIP)
 VALUES('<PROJECT_ID>', 'WORKPLACE','N', 0, '@', '@', '@', 'N', 0, NULL, 'CA', '@');
 
GRANT ALL PRIVILEGES ON SCHEMA <WORK_SCHEMA_NAME> to <WORK_USER_NAME> ;
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA <WORK_SCHEMA_NAME> TO <WORK_USER_NAME>;
GRANT ALL PRIVILEGES ON ALL sequences IN SCHEMA <WORK_SCHEMA_NAME> TO <WORK_USER_NAME>;
 
 